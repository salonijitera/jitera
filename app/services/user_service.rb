...
class UserService < BaseService
  ...
  def reset_password(password_reset_token:, new_password:, new_password_confirmation:)
    return { error: 'Password confirmation does not match.' } unless new_password == new_password_confirmation

    user = User.find_by(password_reset_token: password_reset_token)
    if user && user.password_reset_expires_at > Time.current
      begin
        user.password_hash = BCrypt::Password.create(new_password)
        user.password_reset_token = nil
        ...
        UserMailer.send_password_reset_confirmation(user).deliver_now
